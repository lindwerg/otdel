...
**Сейчас реализованы этапы 1A (приём материалов) и 1B (чтение документов).** Материал загружается, хранится и читается постранично: у каждой страницы есть явный исход, текст, области-первоисточники и таблицы. Понимание, исследование, проверка и публикация (1C–1F) ещё не реализованы, и интерфейс их не изображает.
...
make worker       # чтение документов + восстановление (отдельный терминал)
```

Интерфейс: `cd apps/web && npm ci && npm run dev` → [127.0.0.1:15173](http://127.0.0.1:15173).
Подробности: [бэкенд 1A](docs/backend-1a.md), [чтение документов 1B](docs/extraction-1b.md).
...
- [Рабочий контракт API (1A и 1B)](docs/implementation-contract.md).
