//! Domain entities of phase 1A and their wire representation.
//!
...
///
/// Phase 1A only ever produces `Queued`: nothing in this repository reads a document
/// yet, so a freshly uploaded file is never reported as processed. The remaining
/// variants exist because they are part of the written contract and are written by the
/// 1B extraction worker.
...
/// Kind of queued work. 1A enqueues document extraction only.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum JobKind {
    ExtractDocument,
...
/// Queue state. Phase 1A writes `Queued` only — no worker leases extraction work yet.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum JobStatus {
    Queued,
...
/// Stable idempotency key for the extraction job of a material.
///
/// Re-uploading the same bytes (deduplicated) or pressing “retry” twice therefore
/// cannot create a second queue entry for the same material.
